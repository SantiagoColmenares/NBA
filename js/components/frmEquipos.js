...
export class FrmEquipos extends HTMLElement{
    ...
    saveData = () =>{
        document.querySelector('#guardarData').addEventListener('click',(e) => {
            ...
            let equipo = new Equipo(this.dateToJulian(new Date()),nombre.value,fecha.value,email.value,ciudad.value,fecha.value,presidente.value,logo.files[0].name,totalTitulo.value,conferencia.value,Mascota.files[0].name,division.value);
            equipos.push (equipo);
    ...
    cargarEquipos= ()=>{
        ...
        document.querySelector('#mostrarCarta').innerHTML='';
        let equiposHTML = '';
    ...
    equiposHTML = (equipo)=>{
        ...
        <div class="col-3" >
            <div class="card" style="width: 18rem;">
                    ...
                    <a href="#" class="btn btn-danger borrar-equipo" data-eliminar='[${equipo._id}'>-</a>
                    </div>
            </div>              
        </div>
        `;
    ...
    dateToJulian =(date) => {
        ...
        //console.log(this.convertirUtcAGmt(parseInt(utcMillis.toString(16),16)));
        return utcMillis.toString(16);

    }
    eliminarEquipo = ()=>{
        document.querySelectorAll(".borrar-equipo").forEach((val, id) => {
            console.log(val);
            val.addEventListener("click", (e)=>{
                let data = JSON.parse(e.target.dataset.eliminar);
                console.log(data);
                /*let indiceEliminar = equipos.findIndex( equipo => equipo._id === data[0]);
                equipos.splice(indiceEliminar, 1);*/
